...
class RemixDaoDcaWeekStratStrategy(BaseRemixDaoStrategy):
    ...
    def calculate_range(self, lp_market: UniLpMarket, current_tick: int) -> tuple[Decimal, Decimal, int, int]:
        """
    ...
    def rescale_work(self, row_data: Snapshot):
        ...
        try:
            ...
            allow_rescale, _, _, new_tick_lower, new_tick_upper = self.check_rebalance(lp_market, current_tick)
            ...
            try:
                self.utils.current_position_info, base_used, quote_used, _, base_swap_fee, quote_swap_fee = lp_market.add_liquidity_by_value_ext(
                    new_tick_lower, new_tick_upper, base_fee=self.total_base_fee, quote_fee=self.total_quote_fee)

                self.total_base_swap_fee += base_swap_fee
                self.total_quote_swap_fee += quote_swap_fee
    ...
    def first_lp(self, row_data: Snapshot):
        ...
        if self.params.initial_type == 0:  # all quote
            ...
            (_lower_price, _upper_price, lower, upper) = self.calculate_range(lp_market, current_tick)
            ...
            base_fee, quote_fee = ZERO, ZERO  # Initial swap doesn't have accumulated fees yet
            swapped_base, swapped_quote = ZERO, ZERO 
            base_to_swap, quote_to_swap = ZERO, ZERO # These were for logging
...
def process_for_date(csd: datetime, dsd: date, ded: date, id: str, flip_param_dates: list[datetime]):
    ...
    _aggressive = True
    _compound = False
    _folder_prefix = f"ISAO-tick-B-stable-{token0.name.lower()}{token1.name.lower()}-{quote_token.name.lower()}"
...
if __name__ == "__main__":
    ...
    date_ranges: List[tuple[datetime, date, date, str, list[datetime]]] = [
        ...
        # (datetime(2023, 1, 1, 0, 0, 0), date(2023, 1, 1), date(2025, 12, 31), "", []),
