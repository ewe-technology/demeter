    ...
    def rescale_work(self, row_data: Snapshot):
        ...
        try:
            ...
            try:
                ...
                # final_base, final_quote, base_used_fee, quote_used_fee = self.even_rebalance(lp_market, to_swap_base, to_swap_quote, price=current_price)
...
def process_for_date(csd: datetime, dsd: date, ded: date, id: str, flip_param_dates: list[datetime]):
    ...
    _starting_mark_price = Decimal(4000)
    _is_stable = False
    _tick_spacing = 1 if _is_stable else int(fee * 200)  # 10  # should simply be fee * 200
    _aggressive = True
    _compound = False
    _folder_prefix = f"ISAO-percentage-v2-{token0.name.lower()}{token1.name.lower()}-{"aggressive" if _aggressive else "conservative"}-{quote_token.name.lower()}"
