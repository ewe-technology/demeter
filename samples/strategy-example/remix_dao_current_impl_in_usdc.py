...
def process_for_date(csd: datetime, dsd: date, ded: date, id: str, flip_param_dates: list[datetime]):
    ...
    _aggressive = True
    _compound = False
    _folder_prefix = f"ISAO-normal-{token0.name.lower()}{token1.name.lower()}-{"aggressive" if _aggressive else "conservative"}-{quote_token.name.lower()}"
...
if __name__ == "__main__":
    ...
    date_ranges: List[tuple[datetime, date, date, str, list[datetime]]] = [
        ...
        # (datetime(2023, 1, 1, 0, 0, 0), date(2023, 1, 1), date(2025, 11, 30), "", []),
