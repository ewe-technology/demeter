...
def run_test(bull_params: RemixDAOParams, bear_params: RemixDAOParams, params: TestParams, gp: GlobalParams,
    ...
    try:
        ...
        idle_amount = strat.idle_quote + strat.leftover_quote + (strat.idle_base + strat.leftover_base) * final_price
        print(
            f"idle amount: {idle_amount}, idle_quote: {strat.idle_quote}, idle_base: {strat.idle_base}, leftover_quote: {strat.leftover_quote}, leftover_base: {strat.leftover_base}, final price: {final_price}")
...
def process_for_date(csd: datetime, dsd: date, ded: date, id: str, flip_param_dates: list[datetime]):
    ...
    _aggressive = True
    _compound = False
    _price_range_change = Decimal("0.1") # 0.1, 0.25, 0.5, 0.75
    _folder_prefix = f"Mark-1fix-{token0.name.lower()}{token1.name.lower()}-{(_price_range_change * 100):.0f}-{"aggressive" if _aggressive else "conservative"}-{quote_token.name.lower()}"
...
if __name__ == "__main__":
    ...
    date_ranges: List[tuple[datetime, date, date, str, list[datetime]]] = [
        ...
        (datetime(2021, 5, 13, 0, 0, 0), date(2021, 5, 13), date(2025, 12, 31), "", []),
