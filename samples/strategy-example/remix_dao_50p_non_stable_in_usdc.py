...
from remix_dao_utils import RemixDaoUtils, RemixDAOParams, WeeklyTrigger, performance_metrics_for_dca, is_first_minute_of_hour, near_zero, getPrice
...
class RemixDaoDcaWeekStratStrategy(BaseRemixDaoStrategy):
    ...
    def initialize(self):
        ...
        for year in range(2021, 2027):
            self.triggers.append(
                AtTimeTrigger(time=datetime(year, 1, 1, 0, 0), do=self.yearly_snapshot))
        ...
        match self.params.rescale_frequency:
            case RescaleFrequency.minute1:
                self.triggers.append(PeriodTrigger(time_delta=timedelta(minutes=1), do=self.rescale_work))
    ...
    def yearly_snapshot(self, row_data: Snapshot):
        """
        Takes a snapshot of the current strategy performance metrics.
        """
        # print(f"Yearly snapshot triggered at: {row_data.timestamp}")
        ...
        current_position = self.utils.current_position_info

        ed = RemixDaoUtils.yearly_snapshot(self.gp, lp_market, row_data, current_price, current_tick, current_position,
                                           self.idle_base, self.idle_quote,
                                           self.total_base_fee, self.total_quote_fee)

        self.export_actions.append(ed)



    def calculate_wrap_range(self, lp_market: UniLpMarket, current_tick: int, current_price: Decimal, base: Decimal, quote: Decimal, upper_range: int, lower_range: int) -> tuple[int, int]:
    ...
    def rescale_work(self, row_data: Snapshot):
        ...
        try:
            ...
            minute_rescale = False
            # start = datetime(2022, 1, 1, 0, 0, 0)
            # end = datetime(2022, 1, 1, 5, 0, 0)
            debug = False #start <= row_data.timestamp <= end

            if not is_first_minute_of_hour(row_data.timestamp):

                allow_rescale, new_tick_upper, new_tick_lower = False, 0, 0
                lower, upper = self.utils.current_position_info
                token0, token1 = lp_market.get_position_amount(self.utils.current_position_info)
                lp_base, lp_quote = (token1, token0) if lp_market.pool_info.is_token0_quote else (token0, token1)

                if current_tick <= lower or current_tick >= upper:

                    lp_quote_zero, lp_base_zero = self.is_quote_zero(lp_quote), self.is_base_zero(lp_base)
                    if not lp_quote_zero and lp_base_zero and not self.is_base_zero(self.idle_base):
                        allow_rescale = True
                        pass
                    elif not lp_base_zero and lp_quote_zero and not self.is_quote_zero(self.idle_quote):
                        allow_rescale = True
                        pass

                if debug:
                    print(f"{row_data.timestamp.strftime('%Y-%m-%d %H:%M')} => position: {self.utils.current_position_info} current: {current_tick}, allow_rescale: {allow_rescale}, lp_base({self.is_base_zero(lp_base)}): {lp_base}, lp_quote({self.is_quote_zero(lp_quote)}): {lp_quote}, idle_base({self.is_base_zero(self.idle_base)}): {self.idle_base}, idle_quote({self.is_quote_zero(self.idle_quote)}): {self.idle_quote}")
                minute_rescale = allow_rescale

            else:
                allow_rescale, new_tick_upper, new_tick_lower = self.utils.verify_and_get_new_rescale_tick_boundary(
                    row_data, self.was_in_range, self.last_rescale_tick, use_min_tick_space=True)
            ...
            if debug:
                print(f"{row_data.timestamp.strftime('%Y-%m-%d %H:%M')} => minute_rescale: {minute_rescale}")
            ...
            if minute_rescale:
                pass
            elif (new_tick_lower == self.utils.current_position_info[0] and new_tick_upper ==
            ...
            if debug:
                print(f"{row_data.timestamp.strftime('%Y-%m-%d %H:%M')} => going to rescale ")
            ...
            try:
                ...
                if debug:
                    print(f"{row_data.timestamp.strftime('%Y-%m-%d %H:%M')} => base_removed: {base_removed}, quote_removed: {quote_removed}, base_fee: {base_fee}, quote_fee: {quote_fee}")
            ...
            base_zero = self.is_base_zero(self.idle_base)
            quote_zero = self.is_quote_zero(self.idle_quote)
            base_removed_zero = self.is_base_zero(base_removed)
            quote_removed_zero = self.is_quote_zero(quote_removed)
            if base_removed_zero and not base_zero: # need to merge into one LP around price


                base, quote = self.idle_base + self.leftover_base, quote_removed + self.leftover_quote
                new_tick_lower, new_tick_upper = self.calculate_wrap_range(lp_market, current_tick, current_price, base, quote, self.utils.params.tick_spread_upper, self.utils.params.tick_spread_lower)

                # print(f"merge LP => current tick: {current_tick}, new tick: {new_tick_lower}, {new_tick_upper}, base: {base}, quote: {quote}, current_price: {current_price}")
                self.idle_base = ZERO
                self.idle_quote = ZERO
                self.leftover_base = ZERO
                self.leftover_quote = ZERO
                pass
            elif quote_removed_zero and not quote_zero: # need to merge into one LP around price
                base, quote = base_removed + self.leftover_base, self.idle_quote + self.leftover_quote
                new_tick_lower, new_tick_upper = self.calculate_wrap_range(lp_market, current_tick, current_price, base, quote,
                                                      self.utils.params.tick_spread_upper,
                                                      self.utils.params.tick_spread_lower)
                # print(f"merge LP => current tick: {current_tick}, new tick: {new_tick_lower}, {new_tick_upper}, base: {base}, quote: {quote}, current_price: {current_price}")
                self.idle_base = ZERO
                self.idle_quote = ZERO
                self.leftover_base = ZERO
                self.leftover_quote = ZERO
                pass
            elif base_zero and quote_zero:
                if base_removed_zero:
                    quote = (quote_removed + self.leftover_quote) / 2
                    self.idle_quote = quote
                    self.idle_base = ZERO
                    self.leftover_quote = ZERO
                else:
                    base = (base_removed + self.leftover_base) / 2
                    self.idle_base = base
                    self.idle_quote = ZERO
                    self.leftover_base = ZERO
            else:
                # print(f"current base: {base}, quote: {quote}, idle base: {self.idle_base}, idle quote: {self.idle_quote}")
                pass
            ...
            try:
                ...
                if self.params.compound:
                    ...
                    base_leftover = base - base_used
                    quote_leftover = quote - quote_used
                    self.leftover_base += base_leftover
                    self.leftover_quote += quote_leftover
                ...
                print(f"{row_data.timestamp.strftime('%Y-%m-%d %H:%M')} => failed to add liquidity, current tick: {current_tick}, upper: {new_tick_upper}, lower: {new_tick_lower}, base: {base}, quote: {quote}, idle_base: {self.idle_base}, idle_quote: {self.idle_quote}, minute_rescale: {minute_rescale}, base_removed: {base_removed}, quote_removed: {quote_removed}")
                raise e
    ...
    def first_lp(self, row_data: Snapshot):
        ...
        quote_usdc_price = self.init_quote_usd_price = Decimal(getPrice(self.gp.quote_token.name, row_data.timestamp))#Decimal(INIT_PRICE) #self.get_usdc_price(row_data)
        ...
        # print(f"initial balance => base: {lp_market.broker.get_token_balance(self.gp.base_token)}, quote: {lp_market.broker.get_token_balance(self.gp.quote_token)}")
        ...
        # print(
        #     f"after rebalance balance => base: {lp_market.broker.get_token_balance(self.gp.base_token)}, quote: {lp_market.broker.get_token_balance(self.gp.quote_token)}")
        # print(f"price before: {lp_market.get_market_balance().net_value}")
        # self.utils.current_position_info, _, _, _ = lp_market.add_liquidity_by_tick(lower, upper,
        #                                                                             base_max_amount=lp_market.broker.get_token_balance(self.gp.base_token)/Decimal(2),
        #                                                                             quote_max_amount = lp_market.broker.get_token_balance(self.gp.quote_token) / Decimal(2),)#, tick=current_tick)
        self.utils.current_position_info, _, _, _ = lp_market.add_liquidity_by_tick(lower, upper)#, tick=current_tick)
        # print(
        #     f"after add liquidity balance => base: {lp_market.broker.get_token_balance(self.gp.base_token)}, quote: {lp_market.broker.get_token_balance(self.gp.quote_token)}")
        # print(f"price after: {lp_market.get_market_balance().net_value}")
        print(
    ...
    def calculate_final_result(self, row_data: Snapshot):
        ...
        ed.lp_net_value = lp_market.get_market_balance().net_value
        ed.lp_net_value_with_idle = (self.idle_base * ed.price) + self.idle_quote + ed.lp_net_value
        ed.quote_balance = lp_market.broker.get_token_balance(self.gp.quote_token) #+ self.idle_quote + self.leftover_quote
        ...
        usd_price = self.final_quote_usd_price = Decimal(getPrice(self.gp.quote_token.name, row_data.timestamp))#Decimal(FINAL_PRICE) #ONE #lp_row_data.usdc_price
...
def run_test(bull_params: RemixDAOParams, bear_params: RemixDAOParams, params: TestParams, gp: GlobalParams, processed_data: pd.DataFrame | None, usdc_price_data: pd.DataFrame | None = None) -> Dict[str, Decimal]:
    try:
        ...
        pool = UniV3Pool(gp.token0, gp.token1, gp.fee, gp.quote_token, tick_spacing=bull_params.tick_spacing)  # declare pool, Arbitrum One
        ...
        print(f"idle amount: {idle_amount}, idle_quote: {strat.idle_quote}, idle_base: {strat.idle_base}, leftover_quote: {strat.leftover_quote}, leftover_base: {strat.leftover_base}, final_base_price: {final_base_price}")
        ...
        metrics["idle_base"] = strat.idle_base + strat.leftover_base
        metrics["idle_quote"] = strat.idle_quote + strat.leftover_quote
        metrics["final_base_price"] = final_base_price
        ...
        metrics["in_range_pct"] = Decimal(strat.total_in_range_minutes / strat.total_minutes) if strat.total_minutes > 0 else Decimal(0)
        metrics["in_range_locked_pct"] = Decimal(strat.total_in_range_and_in_lock_minutes/ strat.total_in_range_minutes) if strat.total_in_range_minutes > 0 else Decimal(0)
        metrics["out_range_locked_pct"] = Decimal(strat.total_out_of_range_and_in_lock_minutes/ (strat.total_minutes - strat.total_in_range_minutes)) if (strat.total_minutes - strat.total_in_range_minutes) > 0 else Decimal(0)
        metrics["lock_pct"] = Decimal((strat.total_out_of_range_and_in_lock_minutes + strat.total_in_range_and_in_lock_minutes)/ strat.total_minutes) if strat.total_minutes > 0 else Decimal(0)
        metrics["out_of_fund_date"] = Decimal(strat.out_of_fund_date.timestamp()) if strat.out_of_fund_date is not None else None

        metrics["base_swap_fee"] = strat.total_base_swap_fee
        metrics["quote_swap_fee"] = strat.total_quote_swap_fee
...
## negative tick: token0 is worthless than token1
    ...
    _aggressive = True
    _compound = False
    _folder_prefix = f"ISAO-fix1-{token0.name.lower()}{token1.name.lower()}-{"aggressive" if _aggressive else "conservative"}-{quote_token.name.lower()}"
    ...
    _rescale_frequencies = [RescaleFrequency.minute1, ]
