...
class RemixDaoDcaWeekStratStrategy(Strategy):
    ...
    def __init__(self, _utils: RemixDaoUtils, _params: TestParams, _gp: GlobalParams):
        ...
        # self.refill_percent: Decimal | None = None
        self.refill_total_base: Decimal = ZERO
        self.refill_total_quote: Decimal = ZERO
        self.refill_total_usdc: Decimal = ZERO
    ...
    def check_and_add_dca(self, row_data: Snapshot):
        ...
        if timing == DcaTiming.none:
            return
        # if self.dca_usdc_accumulated <= _ZERO:
        #     return
        ...
        if lp_market.pool_info.is_token0_quote:  # quote is basically USDC
            ...
            if current_tick < current_tick_lower:  # liquidity is all in base token
                ...
                # self.dca_usdc_accumulated = _ZERO
                lower, upper = self.utils.current_position_info[0], self.utils.current_position_info[1]
                created_position, base_used, quote_used, _ = lp_market.add_liquidity_by_tick(lower, upper,
    ...
    def rescale_work(self, row_data: Snapshot):
        ...
        try:
            ...
            refill_base = refill_quote = refill_usdc = ZERO

            if self.gp.refill_percent is not None:
                quote_threshold = self.gp.init_quote * (1 - self.gp.refill_percent)
                if base == ZERO:

                    if quote_threshold > quote:
                        refill_quote = refill_usdc = self.gp.init_quote - quote
                        self.refill_total_quote += refill_quote
                        self.refill_total_usdc += refill_quote

                        self.broker.add_to_balance(self.gp.quote_token, refill_quote)
                        quote += refill_quote
                        pass
                else:
                    base_quote_value = current_price * base
                    if quote_threshold > base_quote_value:
                        refill_base_quote = self.gp.init_quote - base_quote_value
                        refill_usdc = refill_base_quote
                        refill_base = refill_base_quote / current_price
                        self.refill_total_base += refill_base
                        self.refill_total_usdc += refill_base_quote

                        self.broker.add_to_balance(self.gp.base_token, refill_base)
                        base += refill_base
                        pass
            ...
            ed.refill_base = refill_base
            ed.refill_quote = refill_quote
            ed.refill_usdc = refill_usdc
...
def run_test(bull_params: RemixDAOParams, bear_params: RemixDAOParams, params: TestParams, gp: GlobalParams, processed_data: pd.DataFrame | None) -> Dict[str, Decimal]:
    try:
        ...
        metrics: dict[str, Decimal] = performance_metrics_for_dca(
            actuator.account_status_df["net_value"], float(strat.dca_total_added) + float(strat.refill_total_usdc),
        ...
        metrics["total_refill_usd"] = strat.refill_total_usdc
...
def process_for_date(csd: datetime, dsd: date, ded: date, id: str, flip_param_dates: list[datetime]):
    ...
    gp = GlobalParams(token0=token0, token1=token1, fee=fee, init_quote=init_quote,
                      ...
                      refill_percent=_refill_percent,)
...
if __name__ == "__main__":
    ...
    date_ranges: List[tuple[datetime, date, date, str, list[datetime]]] = [
        ...
        (datetime(2021, 5, 5, 0, 0, 0), date(2021, 5, 5), date(2025, 7, 31), "refill", []),
        #  2024/01/01~2025/07/31
        (datetime(2024, 1, 1, 0, 0, 0), date(2024, 1, 1), date(2025, 7, 31), "refill", []),
