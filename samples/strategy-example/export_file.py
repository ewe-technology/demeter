...
class ExportData(object):
    def __init__(self):
        ...
        self.base_added_1: decimal.Decimal | None = None
        self.quote_added_1: decimal.Decimal | None = None
        self.base_added_2: decimal.Decimal | None = None
        self.quote_added_2: decimal.Decimal | None = None
        self.rebalance_price_lower: decimal.Decimal | None = None
        self.rebalance_price_upper: decimal.Decimal | None = None
        self.previous_price_lower: decimal.Decimal | None = None
        self.previous_price_upper: decimal.Decimal | None = None
        ...
        self.lp_net_value_with_idle: decimal.Decimal = ZERO
        self.total_net_value: decimal.Decimal = ZERO
        self.total_net_value_base: decimal.Decimal = ZERO
        ...
        # self.in_range_pct: decimal.Decimal = ZERO
        # self.in_range_locked_pct: decimal.Decimal = ZERO
        # self.out_range_locked_pct: decimal.Decimal = ZERO
        # self.lock_pct: decimal.Decimal = ZERO
...
def export_file(file_path: str, actions: List[ExportData]):
    with open(file_path, 'w') as f:
        ...
        csvwriter.writerow(
            ["time", "price", "total_net_value", "total_net_value_base", "lp_net_value", "lp_net_value_with_idle",
        ...
        for action in actions:
            csvwriter.writerow(
                [action.time.strftime("%Y-%m-%d %H:%M:%S"), to_str(action.price), to_str(action.total_net_value), to_str(action.total_net_value_base),
                 to_str(action.lp_net_value), to_str(action.lp_net_value_with_idle),
...
def export_file_2_position(file_path: str, actions: List[ExportData]):
    with open(file_path, 'w') as f:
        csvwriter = csv.writer(f, delimiter=',', quoting=csv.QUOTE_MINIMAL, quotechar='"')
        csvwriter.writerow(
            ["time", "price", "total_net_value", "total_net_value_base", "lp_net_value", "lp_net_value_with_idle",
             "base_removed", "quote_removed", "base_added", "quote_added",
             "base_added_1", "quote_added_1", "base_added_2", "quote_added_2",
             "rebalance_price_lower", "rebalance_price_upper",
             "previous_price_lower", "previous_price_upper",
             "price_lower", "price_upper",
             "base_balance", "quote_balance",
             "base_fee", "quote_fee", "tick", "new_tick_lower", "new_tick_upper",
             "indicator_value", "tick_lower", "tick_upper",
        ...
        for action in actions:
            csvwriter.writerow(
                [action.time.strftime("%Y-%m-%d %H:%M"), to_str(action.price), to_str(action.total_net_value), to_str(action.total_net_value_base),
                 to_str(action.lp_net_value), to_str(action.lp_net_value_with_idle),
                 to_str(action.base_removed), to_str(action.quote_removed), to_str(action.base_added), to_str(action.quote_added),
                 to_str(action.base_added_1), to_str(action.quote_added_1), to_str(action.base_added_2), to_str(action.quote_added_2),
                 to_str(action.rebalance_price_lower), to_str(action.rebalance_price_upper),
                 to_str(action.previous_price_lower), to_str(action.previous_price_upper),
                 to_str(action.price_lower), to_str(action.price_upper),
                 to_str(action.base_balance), to_str(action.quote_balance),
                 to_str(action.base_fee), to_str(action.quote_fee), to_str(action.tick), to_str(action.new_tick_lower),
                 to_str(action.new_tick_upper),
...
def export_stable_apr_results(file_path: str, metrics: List[Tuple[str, Dict[str, Decimal]]]):
    with open(file_path, 'w') as f:
        csvwriter = csv.writer(f, delimiter=',', quoting=csv.QUOTE_MINIMAL, quotechar='"')
        csvwriter.writerow(
            ["Strategy", "Return", "Rate of Return", "APR", "Max Draw Down", "Sharpe Ratio", "Volatility",
             "Alpha", "Beta",
             "Total Net Value", "LP Net Value", "Idle Total Value", "Idle Base Amount", "Idle Quote Amount",
             "Total Fee", "Fee to Total Net Value",
             "Total Fee Return",
             "Total Swap Fee",
             "Total Base Fee Used in Swap", "Total Quote Fee Used in Swap",
             "Benchmark return rate", "Benchmark APR",
             ...
             # "Total Refill USD",
             "Rescale Count",
             "Upper Rescale Count", "Lower Rescale Count",
             "In Range %",
             "In Range Locked %", "Out Range Locked %",
             "Lock Count", "Locked Time %",
             "Out of Fund Date",
             "Final Base Price",
             "Out of Range Count"
            ])
        ...
        for (strategy, m) in metrics:
            ...
            csvwriter.writerow(
                [strategy, m[MetricEnum.return_value.name], m[MetricEnum.return_rate.name],
                 ...
                 m[MetricEnum.max_draw_down.name], m[MetricEnum.sharpe_ratio.name], m[MetricEnum.volatility.name],
                 m[MetricEnum.alpha.name], m[MetricEnum.beta.name],
                 to_str(m["total_net_value"]), to_str(m["lp_net_value"]), to_str(m["idle_amount"]), to_str(m.get("idle_base", None)), to_str(m.get("idle_quote", None)),
                 to_str(m["total_fee"]), to_str(m["fee_to_total_net_value"]),
                 to_str(m["total_fee_return"]),
                 to_str(m.get("total_swap_fee", None)),
                 to_str(m.get("total_base_swap_fee", None)), to_str(m.get("total_quote_swap_fee", None)),
                 m[MetricEnum.benchmark_rate.name], m[MetricEnum.annualized_benchmark_rate.name],
                 m["spread_mean"], m["spread_median"], m["action_count"], m["benchmark_max_draw_down"],
                 # m["total_dca"], m["dca_count"], to_str(m.get("dca_addon_count", None)),
                 to_str(m.get("total_return_usd", None)),
                 ...
                 # to_str(m.get("short_amount")), to_str(m.get("total_net_value_with_short")), to_str(m.get("total_gl_with_short")),
                 # to_str(m.get("total_refill_usd", None)),
                 to_str(m.get("rescale_count", None)),
                 to_str(m.get("upper_rescale_count", None)), to_str(m.get("lower_rescale_count", None)),
                 to_str(m.get("in_range_pct", None)),
                 to_str(m.get("in_range_locked_pct", None)), to_str(m.get("out_range_locked_pct", None)),
                 to_str(m.get("lock_count", None)), to_str(m.get("lock_pct", None)),
                 timestamp_to_str(m.get("out_of_fund_date", None)),
                 to_str(m.get("final_base_price", None)),
                 to_str(m.get("out_of_range_count", None))
                 ]
            )

    pass

def timestamp_to_str(timestamp: Decimal | None) -> str:
    return datetime.fromtimestamp(float(timestamp)).strftime('%Y-%m-%d %H:%M:%S') if timestamp is not None else ""
