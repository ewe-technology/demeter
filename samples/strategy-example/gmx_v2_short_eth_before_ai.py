...
# _OPEN_SHORT = True
PRINT_PRICE = False
POSITION_OPEN_STRATEGY = IndicatorType.STANDARD.value
_FEE_PERCENT = ZERO
_DELAY_ENTRY = False
...
class GmxV2LpStrategy(Strategy):
    ...
    def __init__(self, macd_config: MacdConfig = MacdConfig(), rsi_config: StockRsiConfig = StockRsiConfig(),
                 to_short: bool = True, to_long: bool = True):
        super().__init__()
    ...
    def initialize(self):
        ...
        if POSITION_OPEN_STRATEGY == IndicatorType.MACD.value:
        ...
        elif POSITION_OPEN_STRATEGY == IndicatorType.RSI.value:
            resampled_prices = resample_data(market_data.longPrice, self.macd_config.sample_period)

            rsi_fastk, rsi_fastd, rsi_diff = stochRSI(resampled_prices, self.rsi_config.fastk_period,
                                                      self.rsi_config.fastd_period, self.rsi_config.fastd_matype)
            # print(f"{macd_data.keys()}")
            # print(f"{macd_signal.keys()}")
            # print(f"{macd_hist.keys()}")
            self.add_column(MARKET_KEY, "rsi_fastk", macd_data)
            self.add_column(MARKET_KEY, "rsi_fastd", macd_signal)
            self.add_column(MARKET_KEY, "rsi_diff", macd_delta)
...
if __name__ == "__main__":
    ...
    strategy_str = config_file.get("position_open_strategy")
    # Map the string from config to the appropriate enum value
    if strategy_str == IndicatorType.STANDARD.value:
        POSITION_OPEN_STRATEGY = IndicatorType.STANDARD.value
    elif strategy_str == IndicatorType.MACD.value:
        POSITION_OPEN_STRATEGY = IndicatorType.MACD.value
    elif strategy_str == IndicatorType.RSI.value:
        POSITION_OPEN_STRATEGY = IndicatorType.RSI.value
    else:
        POSITION_OPEN_STRATEGY = IndicatorType.STANDARD.value
